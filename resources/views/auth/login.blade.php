@extends('layouts.main', ['class' => 'off-canvas-sidebar', 'activePage' => 'login', 'title' => __('Booking System')])
    ...
    <div class="col-lg-4 col-md-6 col-sm-8 ml-auto mr-auto">
      <form class="form" method="POST" action="{{ route('login') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body">
            <p class="card-description text-center mb-4"></p>
            ...
            <div class="bmd-form-group{{ $errors->has('username') ? ' has-danger' : '' }}">
              <div class="input-group">
                ...
                <input type="text" name="username" class="form-control" placeholder="{{ __('Usuario o Email...') }}" value="{{ old('username', null) }}" required autocomplete="username" autofocus>
              </div>
        ...
        <div class="col-6">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-light">
                    <h6>{{ __('Olvido su contraseña?') }}</h6>
                </a>
            @endif
        </div>
        <div class="col-6 text-right">
            <a href="{{ route('register') }}" class="text-light">
                <h6>{{ __('Crear una cuenta') }}</h6>
            </a>
        </div>
      </div>
    </div>
